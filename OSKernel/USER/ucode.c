...
char buf[128];
char *cmd[]={"getpid", "ps", "chname", "fork", "switch", "wait", "exit", "kgetc", "kputc", "exec", "pipe", "pfd", "close", "read", "write", 0};
...
{
	printf("*********************** Menu *************************************************************\n");
	printf("* getpid ps  chname  fork  switch  wait  exit kgetc kputc exec pipe pfd close read write *\n");
	printf("******************************************************************************************\n");
}
...
int write()
{
	...
	return syscall(14, fd, buf, strlen(buf));
}
